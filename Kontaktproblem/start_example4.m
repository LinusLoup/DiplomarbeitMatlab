...
vol_load_y = @(x,y) load2(x,y);
    ...
    function z = load3(x,y)
        ...
        index = find(x>-0.1 & x<0.1);
        z(index) = -1000*ones(size(index));
    end
...
u_S = [];
itermax = 1;          % maximum iteration depth
nmax = 1000000;          % maximum number of nodes
eps = 0.001;           % upper bound for the hierarchical error estimate
theta_rho = 0.7;      % contraction parameter for local contributions of the error estimate
theta_osc = 0.3;      % contraction parameter for local contributions of the oscillations

tic
% adaptive algorithm:
[u_S,p,e,t,midtri,midpoints,rhoS_plot,IQ_plot,J_error,osc_term, osc1_term,osc2_term,recursion_depth, degree_of_freedom] = adaptive_refinement_solution(p,e,t,lambda,mu,u_S,vol_load_x, vol_load_y,surf_load_x,surf_load_y,my_obstacle, data,J_u,eps,theta_rho,theta_osc,nmax,itermax);
toc
