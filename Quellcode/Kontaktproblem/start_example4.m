...
theta_rho = 0.35;      % contraction parameter for local contributions of 
                       % the error estimate

tic
% adaptive algorithm:
[u_S,p,e,t,rhoS_plot,IQ_plot,J_error,recursion_depth,degree_of_freedom,...
    time] = adaptive_refinement_solution(p,e,t,lambda,mu,u_S,vol_load_x,...
    vol_load_y,surf_load_x,surf_load_y,point_load,my_obstacle,data,J_u,...
    eps,theta_rho,nmax,itermax);
toc
...
pdeplot(p,e,t,'xydata',u_S(1,:),'zdata',u_S,'mesh','off','colormap', 'jet',...
    'colorbar','on');
title('solution (in x-direction) of the obstacle problem','FontSize',15)

figure(recursion_depth+4);
pdeplot(p,e,t,'xydata',u_S(2,:),'zdata',u_S,'mesh','off','colormap', 'jet',...
    'colorbar','on');
