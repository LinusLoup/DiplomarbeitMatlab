...
%N0PLUSMINUS evaluates the sets N^{0+} and N^{0-} for the calculation of 
%the oscillationterms, given the inner contact nodes N^0_set, the nodes of 
%the mesh, the triangle ordering, the edges and the midpoints-triangle 
%ordering, the loadfunction, the obstacle and vector of u_S values.
...
% calculating the points of the reference element to compare the 
% functionvalues later:
...
for i = 1:length(N0_set)
    ...
    for j = 1:length(w_p)
        ...
        % If any z-value is less than zero, the point cannot be an element 
        % of N^{0+}, that is, why flag_plus = 1. If all f_load-values and 
        % all z-values are less than zero, p could be in N^{0-}:
    ...
    % verification of j_E <= 0 for all E in E_p: evaluating of the set of 
    % edges E_p, which contains the point p:
    ...
    % verificating, if there is full contact and f<=0 and j_E<=0 for all E 
    % in E_p:
