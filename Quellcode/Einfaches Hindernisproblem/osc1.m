function [osc1_val,osc1_vec] = osc1(N0plus_set,obstacle_values,nodes,...
    triangles,uS_values)
%OSC1 evaluates the obstacle oscillation osc_1(u_S,phi). The general
...
for i = 1:length(N0plus_set)
    ...
    int_h = 0;
    % evaluating the support of the shape funktions and the indix of the
    ...
    for j = 1:length(w_p)
        ...
        grad_u = [x(2)-x(1), y(2)-y(1); x(3)-x(1), y(3)-y(1)]*grad_u';
            ...
            % to integrate the gradients, we just multiplicate the area of 
            % the reference element with the high (the functionvalue), 
